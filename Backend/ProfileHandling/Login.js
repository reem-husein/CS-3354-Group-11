...
router.post("/register", async (req, res) => {
  const { employeeUpload: {email, password} } = req.body;

  try {
    let driver = await Driver.findOne({ "employeeUpload.email": email });
    if (driver) return res.status(400).json({ msg: "User already exists" });

    const salt = await bcrypt.genSalt(10);
    const hashedPassword = await bcrypt.hash(password, salt);

    driver = new Driver({ employeeUpload: { email, password: hashedPassword } });
    await driver.save();
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    const driver = await Driver.findOne({ "employeeUpload.email": email });
    ...
    const isMatch = await bcrypt.compare(password, driver.employeeUpload.password);
    if (!isMatch) return res.status(400).json({ msg: "Invalid credentials" });

    // Return a response with the driver's _id and email from the employeeUpload object
    res.json({
      msg: "Login successful",
      user: { id: driver._id, email: driver.employeeUpload.email },
    });
  } catch (err) {
    console.error(err.message);
    res.status(500).send("Server error");
